...
class Tab: NSObject {
  ...
  private var secureContentState: TabSecureContentState {
    get async {
      guard let webView = webView, let url = await webView.url else {
        return .unknown
      }
      ...
      let isMixedContent = await !webView.hasOnlySecureContent
      if url.scheme == "http" {
        return .missingSSL
      }
      if let serverTrust = await webView.serverTrust,
